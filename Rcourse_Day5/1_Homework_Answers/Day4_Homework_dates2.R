...
setwd("P:/OSUcourse2015/Rcourse_Day5/1_Homework_Answers")
...
#Read seed data
 ...
 doses <- read.csv("doses2.csv", stringsAsFactors=F)
 head(doses)
...
#Calculate TAFD
    ...
    #It's a time object - we will convert it to a number
...
#Write to a file and we are done!
  write.csv(blankdata, file="nmdata_sim_75mgqd_5days_21daycycle.csv", na=".", row.names=F, quote=F)
...
##Answer:
#See doses2.csv
#We removed the 20:00 evening doses
#We also removed the C1D6 and C1D7 doses.
#Finally, we changed the dose amount from 50 to 75 mg.

#In the script, all that was needed was to change the input and output filenames.
