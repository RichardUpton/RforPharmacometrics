...
#Set the working directory
 ...
 setwd("P:/OSUcourse2015/Rcourse_Day2/1_Homework_Answers")
...
##Answer: Each column had at least one character (. or #) that was not recognised as a number
  ...
  ##Answer: Sort places a vector in order, unique does not show duplicated values
  ##        These functions were nested here.  Sometimes this is quick, sometimes it is hard to read
...
#Draw some index plots to look for outlier values of covariates
  ...
  ##Answer: These look reasonable for adult weights in kg

  plot(AGE ~ ID, data=covdata)
  #Are these reasonable?  What are the later subjects with the higher ID's all so young?
  ##Answer: These look reasonable, and we don't know why the later subject are young. Let's fire an email back to the company/CRO

  plot(SEX ~ ID, data=covdata)
  #The sex codes are 0 and 1, but which are male and which are female?
  #Is there enough subjects of SEX==0 to use SEX as a covariate?
  ##Answer: Unless we are given the sex code, we can't tell who is male or female!
  ##Only 5 subjects of 1 sex - probably not enough to use sex as a covariate (in most circumstances!)
...
#What is the relationship between the covariates?
  ...
  ##Answer: No, they are not that correlated.
  ##Highly correlated covariates might include WEIGHT, BSA, HEIGHT that are all a metric for body size.
...
#Please email/upload these summary statistics to Dr Phelps.
