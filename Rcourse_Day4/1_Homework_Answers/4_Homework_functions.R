...
#Now, set our working directory - files are read and write here
  setwd("P:/OSUcourse2015/Rcourse_Day4/1_Homework_Answers")
...
#We need to write a function thats calculates MAP as a percent of baseline for each subject.
...
#----------------------------------------------------------------------------
#Well, that's a pretty neat looking plot.
...
##Answer.  Plot1 show us on average how fentanyl changes blood pressure from a baseline value.  If we repeated the study 1000
# times, the true change in MAP would be somewhere within the bounds given by the sem (sem*1.96 for 95% certainty).  On average
# fentanyl in this simulated study (not real data!) does not change MAP from baseline.
# Plot 1 doesn't tell us anything about the variability of changes in MAP with fentanyl.

# For interpreting our PK study, another issue is probably of more importance.  Where there any subjects with unusually low or
# high MAP that might have affected the disposition of fentanyl?  MAP may indicate a hypo- or hyper-dynamic circulation.
# We don't get this information from Plot 1
...
##Answer.  By expressing MAP as a percent of baseline, we remove all information about the distribution of the baseline MAP.
# Plot 3 shows us that 2 subjects had unusually high or low MAP before the fentanyl was given.
...
##Answer. More than 1 way to do this!  Here is one way.
  MAPstats2 <- summaryBy(MAP~ID, data=nmdata, FUN=min)
  MAPstats2
...
##Answer. More than 1 way to do this!  Again! Here is one way.
  MAPstats3 <- summaryBy(MAP~ID, data=nmdata[nmdata$TIME==0,], FUN=min)
  MAPstats3

  MAPstats3$LowMAP <- MAPstats3$MAP.min < 60
  MAPstats3

#Looks like no subjects had a low baseline MAP.
...
##Answer.  Using sem for error bars doesn't tell the reader about the variability of changes in MAP in this study.
##Error bars based on standard deviation will show this.

  
    plotobj4 <- ggplot(MAPstats) + theme_bw()
    maintext <- "Mean Arterial Blood Pressure\nmean and SD\n"
    plotobj4 <- plotobj4 + geom_point(aes(x=TIME, y=mean),  size=1.5)  
    plotobj4 <- plotobj4 + geom_line(aes(x=TIME, y=mean), size=0.5, linetype="dashed")
    plotobj4 <- plotobj4 + geom_errorbar(aes(x=TIME, ymax = mean + sd, ymin=mean - sd), size=0.5, width=0, position=position_dodge(width=3) )
    plotobj4 <- plotobj4 + geom_hline(yintercept=100, linetype="dotted")
    plotobj4 <- plotobj4 + scale_x_continuous("Time after dose (min)",lim=c(0,600), breaks=seq(0,600,by=60)) 
    plotobj4 <- plotobj4 + scale_y_continuous("MAP(% baseline)", lim=c(50,150))
    plotobj4 <- plotobj4 + ggtitle(maintext)
    plotobj4

    ggsave("plot4.png", width=5,height=4)


# But to summarize what happened to MAP in this study, it would be better to plot median and 90% CI.  Or use Plot 3 with a smooth.

   plotobj5 <- ggplot(plotdata) + theme_bw()
   plotobj5 <- plotobj5 + geom_line(aes(x=TIME,y=MAP, colour=Subject))
   plotobj5 <- plotobj5 + scale_x_continuous("Time after dose (min)",lim=c(0,600), breaks=seq(0,600,by=60))
   plotobj5 <- plotobj5 + geom_smooth(aes(x=TIME,y=MAP), method=loess, span=1, se=T, colour="black", size=1)
   plotobj5 <- plotobj5 + scale_y_continuous("MAP(mmHg)", lim=c(50,180))
   plotobj5 <- plotobj5 + theme(legend.position="none") 
   plotobj5

   ggsave("plot5.png", width=5,height=4)
